...
class _CraneMonitoringScreenState extends State<CraneMonitoringScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('CraneIQ - Zone Control'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text('Pro Plan', style: TextStyle(color: Colors.blue[700], fontSize: 14, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
        ],
      ...
      drawer: Sidebar(onItemSelected: (title) {
        if (!mounted) return;
        if (title == 'Zone Control') {
          // Already on this page, no action needed
        } else if (!['Rule Engine', 'Vibration Monitoring', 'Error Log', 'Load', 'Data Hub', 'Reports', 'Alerts', 'Settings', 'Operations Log', 'Brake Monitoring', 'Temperature'].contains(title)) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Tapped $title (frontend only)'),
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildControlButton(
                            ...
                            onPressed: () {
                              setState(() {
                                cameraZoom = (cameraZoom * 1.2).clamp(0.5, 3.0);
                              });
                            },
                            tooltip: 'Zoom In',
                          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            value,
            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.blue),
            textAlign: TextAlign.center,
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text('$value m', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.blue)),
        ],
      ),
    );
  }
...
class Crane3DPainter extends CustomPainter {
  final double bridgeX;
  final double trolleyZ;
  final double hookY;
  ...
  void drawBox(Canvas canvas, Paint paint, Size size, double x, double y, double z, double w, double h, double d) {
    ...
    canvas.drawLine(vertices[0], vertices[1], paint);
    canvas.drawLine(vertices[1], vertices[5], paint);
    canvas.drawLine(vertices[5], vertices[4], paint);
    canvas.drawLine(vertices[4], vertices[0], paint);
    
    canvas.drawLine(vertices[3], vertices[2], paint);
    canvas.drawLine(vertices[2], vertices[6], paint);
    canvas.drawLine(vertices[6], vertices[7], paint);
    canvas.drawLine(vertices[7], vertices[3], paint);
    
    canvas.drawLine(vertices[0], vertices[3], paint);
    canvas.drawLine(vertices[1], vertices[2], paint);
    canvas.drawLine(vertices[5], vertices[6], paint);
    canvas.drawLine(vertices[4], vertices[7], paint);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    paint.color = Colors.green.withAlpha(51); // 0.2 opacity
    drawBox(canvas, paint, size, 0, 3, 0, 20, 6, 15);
    
    paint.color = Colors.red.withAlpha(51); // 0.2 opacity
    drawBox(canvas, paint, size, 25, 3, 5, 8, 6, 8);
    
    paint.color = Colors.orange.withAlpha(51); // 0.2 opacity
    ...
    paint.color = Colors.red;
    canvas.drawLine(
      project3D(0, 0, 0, size),
      project3D(10, 0, 0, size),
      paint,
    );
    ...
    canvas.drawLine(
      project3D(0, 0, 0, size),
      project3D(0, 0, 10, size),
      paint,
    );
  }

  @override
  bool shouldRepaint(Crane3DPainter oldDelegate) => true;
}