  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: Sidebar(onItemSelected: (title) {
        if (!mounted) return;
        if (!['Rule Engine', 'Vibration Monitoring', 'Error Log', 'Load', 'Data Hub', 'Reports', 'Alerts', 'Settings', 'Operations Log', 'Zone Control', 'Brake Monitoring', 'Temperature'].contains(title)) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Tapped $title'), duration: const Duration(milliseconds: 800)),
          );
        }
      }),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, color: const Color(0xFF1E3A5F), size: 16),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
