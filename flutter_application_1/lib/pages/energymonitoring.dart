...
class _EnergyMonitoringDashboardState extends State<EnergyMonitoringDashboard> {
  ...
  Widget _buildEnergyGauge(bool isMobile) {
    double currentEnergy = filteredTableData.isNotEmpty ? filteredTableData.last['energy'] : 0.0;
                    ...
                    GaugeAnnotation(
                      widget: Column(
                        ...
                        children: [
                          ...
                          const Text('kWh', style: TextStyle(fontSize: 14, color: Colors.black54)),
                        ],
                      ),
                      positionFactor: 0.5,
                      angle: 90,
  ...
  Widget _buildKPICards(bool isMobile) {
    final List<Map<String, dynamic>> kpis = [
      {'value': '24.8 kW', 'label': 'Total Power', 'color': Colors.blue, 'icon': Icons.bolt},
      {'value': '\$18.42', 'label': 'Hourly Cost', 'color': Colors.green, 'icon': Icons.attach_money},
      {'value': '87%', 'label': 'Efficiency', 'color': Colors.orange, 'icon': Icons.trending_up},
      {'value': '1.2 kWh/T', 'label': 'Per Ton', 'color': Colors.purple, 'icon': Icons.scale},
    ];
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  kpi['value'] as String,
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: kpi['color'] as Color),
                ),
                const SizedBox(height: 4),
                Text(kpi['label'] as String, style: const TextStyle(fontSize: 12, color: Colors.black54)),
              ],
            ),
          ),
        );
  ...
  Widget _buildFilters(bool isMobile) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Filters', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E3A5F))),
          const SizedBox(height: 16),
          Column(
            children: [
      ...
      children: [
        ...
        Container(
          ...
          child: DropdownButtonFormField<String>(
            ...
            items: options.map((option) => DropdownMenuItem(value: option, child: Text(option, style: const TextStyle(fontSize: 14)))).toList(),
            onChanged: (newValue) {
              if (newValue == null) return;
              setState(() {
                switch (label) {
                  case 'Crane': selectedCrane = newValue; break;
                  case 'Motor Type': selectedMotorType = newValue; break;
                  case 'Date Range': selectedDateRange = newValue; break;
                  case 'Metric': selectedEnergyMetric = newValue; break;
                }
              });
            },
          ),
    ...
    return LineChart(
      LineChartData(
        ...
        titlesData: FlTitlesData(
          leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 40, getTitlesWidget: (v, m) => Text('${v.toInt()} kW', style: const TextStyle(fontSize: 10, color: Colors.black54)))),
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 20, getTitlesWidget: (v, m) {
            ...
            return v.toInt() < hours.length ? Text(hours[v.toInt()], style: const TextStyle(fontSize: 10, color: Colors.black54)) : const Text('');
          })),
        ),
        borderData: FlBorderData(show: false),
        lineBarsData: [
          LineChartBarData(
            spots: const [FlSpot(0, 12), FlSpot(1, 15), FlSpot(2, 18), FlSpot(3, 14), FlSpot(4, 16), FlSpot(5, 13)],
            isCurved: true,
  ...
  Widget _buildEnergyChart() {
    return BarChart(
      BarChartData(
        ...
        gridData: FlGridData(show: true, drawHorizontalLine: true),
        titlesData: FlTitlesData(
          leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 40, getTitlesWidget: (v, m) => Text('${v.toInt()} kWh', style: const TextStyle(fontSize: 10, color: Colors.black54)))),
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 20, getTitlesWidget: (v, m) {
            final days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
            return v.toInt() < days.length ? Text(days[v.toInt()], style: const TextStyle(fontSize: 10, color: Colors.black54)) : const Text('');
          })),
        ),
        borderData: FlBorderData(show: false),
        barGroups: [
  ...
  Widget _buildCostChart() {
    return LineChart(
      LineChartData(
        gridData: FlGridData(show: true, drawHorizontalLine: true),
        titlesData: FlTitlesData(
          leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 40, getTitlesWidget: (v, m) => Text('\$${v.toInt()}', style: const TextStyle(fontSize: 10, color: Colors.black54)))),
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 20, getTitlesWidget: (v, m) {
            final hours = ['8:00', '10:00', '12:00', '14:00', '16:00', '18:00'];
            return v.toInt() < hours.length ? Text(hours[v.toInt()], style: const TextStyle(fontSize: 10, color: Colors.black54)) : const Text('');
          })),
        ...
        lineBarsData: [
          LineChartBarData(
            spots: const [FlSpot(0, 2.5), FlSpot(1, 3.2), FlSpot(2, 4.1), FlSpot(3, 3.4), FlSpot(4, 3.8), FlSpot(5, 2.9)],
            isCurved: true,
            color: Colors.green,
            barWidth: 3,
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: Colors.black.withOpacity(0.05),
            blurRadius: 10,
            offset: const Offset(0, 2), // FIXED: was 'ач2'
          ),
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Energy Data', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E3A5F))),
          const SizedBox(height: 16),
                  ...
                  DataCell(
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(6),
                        border: Border.all(color: data['status'] == 'NORMAL' ? Colors.green.shade200 : Colors.orange.shade200),
                      ),
                      child: Text(
                        data['status'],
                        style: TextStyle(
